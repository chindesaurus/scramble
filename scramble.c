...
/* function prototypes */
void clear(void);
bool crawl(char *letters, int x, int y);
void draw(void);
bool find(char *s);
...
main(int argc, char **argv) {
    ...
    // allocate memory for user-inputted string
    char *s = malloc((LETTERS + 1) * sizeof(*s));
    assert(s != NULL);
...
/**
 * Clears screen with ANSI escape sequences.
 */
void clear() {
    printf("\033[2J");
    printf("\033[%d;%dH", 0, 0);
 ...
 */
bool 
crawl(char *letters, int x, int y) {
    ...
    for (int i = -1; i <= 1; i++) {
        ...
        for (int j = -1; j <= 1; j++) {
            // recurse: check grid[x + i][y + j] for next letter
 ...
 * (including leading and trailing whitespace). Capitalizes each character 
 * of the string, then null-terminates buffer. 
 */ 
void
getInput(char *buffer) {
   
    int length = 0;             // number of characters in buffer 
    int capacity = LETTERS + 1; // capacity of buffer 
...
lookup(char *s) {
    ...
    // halve array until the bounds overlap
    while (min <= max) {
        
        // define middle, and use it as a point of comparison
        int middle = (max + min) / 2;
        int comparison = strcmp(s, dictionary.words[middle].letters);
 
        // if s is found, mark it as found and return true 
        if (comparison == 0 && dictionary.words[middle].found == false) {
